...
| Guide | Purpose |
| --- | --- |
| [Usage Guide](usage/README.md) | How to inject clocks into application services and choose `now()` or `today()`. |
| [Timezone Guide](timezones/README.md) | UTC defaults, `SystemClock` timezone configuration, and date-boundary guidance. |
| [Testing Guide](testing/README.md) | How to use `FixedClock` and `MockClock` for deterministic tests. |
